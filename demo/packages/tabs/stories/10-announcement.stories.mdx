...
There we go, Alfred's component is already done, so let's write some documentation. See the [live Alfred tabs documentation page](?path=/docs/intro-tabs-example--default-story). You can see the full working version of `alfred-tabs` [here](TODO).
...
You can check them out in our [documentation](TODO), and we'll go into more depth about Lion's additional systems in future blogposts.
